...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .addField('\n\u200b\nHow to Join Server?','You can join the server by entering ** Prime City Roleplay ** at ***PLAY*** or by **139.99.124.144:30125** in ***History***.At the bottom is the server status to see how many people are online and in the queue.',false)
    if (STATUS !== undefined)
    {
  ...
  bot.on('ready',() => {
    ...
    // bot.user.setGame('TIRS RP', 'https://www.youtube.com/channel/UCBacww8c8qLY6Mq5l01hZew');
    bot.user.setActivity('Prime City Roleplay',{type:'PLAYING'}).catch(console.error);
    ...
    if (!message.author.bot) {
      if (message.member) {
        ...
        if (message.channel.id === BUG_CHANNEL) {
          ...
          .setColor(0x2894C2)
          .setTitle('Bug Report')
          .setDescription('Your message has been successfully sent to the staff team!')
          .setTimestamp(new Date());
